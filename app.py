...
def register():
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as cur:
                cur.execute('INSERT INTO users (username,email,password,verified) VALUES (%s,%s,%s,1) RETURNING id',(username,email,hashed))
...
def admin_dashboard():
    ...
    for u in users:
        ...
        if banned:
            ...
            action = f'<form action="/admin/verify/{u["id"]}" method="post" style="display:inline"><button class="btn-resume" style="font-size:12px;padding:6px 12px">Activer</button></form>'
