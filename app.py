...
def send_message(receiver_id):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            ''', (user['id'], receiver_id))
            if not cur.fetchone():
                return jsonify({'error': 'Tu dois être ami avec cet utilisateur pour lui écrire.'}), 403
    ...
    # Limites quotidiennes pour les non-admins
    if not is_admin(user):
        today = datetime.datetime.utcnow().date()
        with get_db() as conn:
            with conn.cursor() as cur:
                # Compte messages du jour
                cur.execute('''SELECT COUNT(*) as c FROM messages
                    WHERE sender_id=%s AND created_at >= %s AND group_id IS NULL''',
                    (user['id'], today))
                msg_count = cur.fetchone()['c']
                if msg_count >= 50:
                    return jsonify({'error': 'Limite de 50 messages par jour atteinte.'}), 429

                if media_type in ('image', 'gif'):
                    cur.execute('''SELECT COUNT(*) as c FROM messages
                        WHERE sender_id=%s AND created_at >= %s AND media_type IN ('image','gif')''',
                        (user['id'], today))
                    if cur.fetchone()['c'] >= 8:
                        return jsonify({'error': 'Limite de 8 images par jour atteinte.'}), 429

                if media_type == 'video':
                    cur.execute('''SELECT COUNT(*) as c FROM messages
                        WHERE sender_id=%s AND created_at >= %s AND media_type='video' ''',
                        (user['id'], today))
                    if cur.fetchone()['c'] >= 1:
                        return jsonify({'error': 'Limite de 1 vidéo par jour atteinte.'}), 429
    if not content and not media_data:
        return jsonify({'error': 'Message vide.'}), 400
    if content and len(content) > 500:
        return jsonify({'error': 'Message trop long.'}), 400
...
def send_group_message(group_id):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute('SELECT 1 FROM group_members WHERE group_id=%s AND user_id=%s', (group_id, user['id']))
            if not cur.fetchone(): return jsonify({'error': 'Non membre.'}), 403
            # Vérifie si un admin est dans le groupe
            cur.execute('''SELECT u.username FROM users u
                JOIN group_members gm ON gm.user_id=u.id
                WHERE gm.group_id=%s''', (group_id,))
            members_in_group = [r['username'].lower() for r in cur.fetchall()]
            group_has_admin = any(m in ADMIN_USERS for m in members_in_group)
    ...
    # Limites seulement si pas admin et groupe sans admin
...
def admin_dashboard():
    ...
    if error_msg:
        flash_msg = f'<div class="error-box" style="margin-bottom:16px">{error_msg}</div>'
    if success_msg:
        flash_msg = f'<div style="background:#142d1e;border:1px solid #1a5e35;color:#4ade80;border-radius:8px;padding:10px 14px;font-size:13px;margin-bottom:16px">{success_msg}</div>'
    ...
    for u in users:
        ...
        ban_action = f'<form action="/admin/unban/{u["id"]}" method="post" style="display:inline"><button class="btn-secondary">Débannir</button></form>' if banned else f'<form action="/admin/ban/{u["id"]}" method="post" class="ban-form"><input type="number" name="duration" value="10" min="1" max="1440"><span style="font-size:11px;color:var(--text-dim)">min</span><button class="btn-danger">Bannir</button></form>'
        delete_btn = f'<form action="/admin/delete-user/{u["id"]}" method="post" style="display:inline" onsubmit="return confirm(\'Supprimer {u["username"]} ?\')"><button class="btn-danger" style="margin-left:6px">Supprimer</button></form>'
        rows_html += f'<tr><td class="date-cell">{u["id"]}</td><td class="username-cell">{u["username"]}</td><td class="email-cell">{u["email"]}</td><td>{badge}</td><td class="date-cell">{str(u["created_at"])[:10] if u["created_at"] else ""}</td><td style="display:flex;gap:4px;flex-wrap:wrap">{ban_action}{delete_btn}</td></tr>'
    ...
    html = f"""<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Dashboard</title>{ADMIN_CSS}
<style>
...
.btn-create{{background:var(--purple);color:#fff;border:none;border-radius:8px;padding:9px 18px;font-size:13px;font-weight:600;font-family:inherit;cursor:pointer}}
...
<form action="/admin/create-user" method="post" class="create-user-form">
  ...
  <div><label>Mot de passe</label><input type="password" name="password" placeholder="min. 6 caractères" required minlength="6"></div>
...
def admin_create_user():
    ...
    if error:
        return redirect(f'/admin?error={error}')
    return redirect('/admin?success=Utilisateur créé avec succès !')
