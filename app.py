...
def register():
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as cur:
                cur.execute('INSERT INTO users (username,email,password) VALUES (%s,%s,%s) RETURNING id',(username,email,hashed))
                user_id = cur.fetchone()['id']
                cur.execute('INSERT INTO verify_tokens (user_id,token) VALUES (%s,%s)',(user_id,verify_token))
            conn.commit()
        return jsonify({'success':True,'verify_token':verify_token,'email':email})
    except Exception:
...
def verify_email():
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute('SELECT * FROM verify_tokens WHERE token=%s',(token,))
            row = cur.fetchone()
            if not row: return jsonify({'error':'Lien invalide.'}), 400
            cur.execute('UPDATE users SET verified=1 WHERE id=%s',(row['user_id'],))
            cur.execute('DELETE FROM verify_tokens WHERE token=%s',(token,))
        conn.commit()
...
def login():
    ...
    if not user['verified']:
        return jsonify({'error':'Vérifie ton email avant de te connecter.'}), 403
    if is_banned(user):
...
@app.route('/api/me')
def me():
    user = check_token(request.headers.get('X-Token'))
    if not user: return jsonify({'error':'Non authentifié.'}), 401
    ok, msg = check_server()
    if not ok: return jsonify({'error':msg,'server_status':server_state['status']}), 503
    if is_banned(user): return jsonify({'error':'Tu es banni temporairement.'}), 403
...
def get_messages():
    ...
    ok, msg = check_server()
    if not ok: return jsonify({'error':msg,'server_status':server_state['status']}), 503
    if is_banned(user): return jsonify({'error':'Tu es banni.'}), 403
    since = request.args.get('since', 0)
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute('SELECT id,username,content,created_at FROM messages WHERE id>%s ORDER BY id ASC LIMIT 100',(since,))
            rows = cur.fetchall()
    return jsonify({'messages':[{'id':r['id'],'username':r['username'],'content':r['content'],'created_at':r['created_at'].isoformat()} for r in rows]})
...
def send_message():
    ...
    if len(content) > 500: return jsonify({'error':'Trop long.'}), 400
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute('INSERT INTO messages (user_id,username,content) VALUES (%s,%s,%s)',(user['id'],user['username'],content))
        conn.commit()
    return jsonify({'success':True})
...
# ── Dashboard admin (HTML intégré) ────────────────────────────────────────────
ADMIN_PASSWORD = os.environ.get('ADMIN_PASSWORD', 'admin1234')

ADMIN_CSS = """
<style>
:root{--bg:#0e0c1a;--bg2:#16132a;--bg3:#1e1a35;--purple:#7c5cbf;--purple-l:#a07ee0;--purple-dim:#3d2e6e;--red:#c0392b;--red-dim:#7a1f16;--green:#27ae60;--amber:#d4a017;--text:#e8e3f0;--text-dim:#9b93b5;--border:#2e2850;}
...
def admin_dashboard():
    ...
    for u in users:
        ...
        if banned:
            ...
            badge = '<span class="badge badge-ok">Vérifié</span>'
...
<div class="stats">
  ...
  <div class="stat-card"><div class="label">Messages</div><div class="value">{msg_count}</div></div>
...
def admin_login():
    ...
    html = f"""<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Admin Login</title>{ADMIN_CSS}</head><body>
...
{error}<form method="post"><label>Mot de passe</label><input type="password" name="password" autofocus><button class="btn-main">Se connecter</button></form>
</div></div></body></html>"""
    return Response(html, mimetype='text/html')
